...
- [ ] 2. RentalRepository 与存储层测试
  - [ ] 2.1 RentalRepository CRUD
...
- [ ] 3. RentalService 收租记账
  - [ ] 3.1 recordRent 原子事务
    - 生成 rent_record + 「租金收入」income 流水（记入用户指定账户），失败回滚
  - [ ] 3.2 deleteRentRecord 原子事务
    - 删除收租记录并同步删除关联流水，失败回滚
  - [ ] 3.3 编写收租记账单元测试

- [ ] 4. IPC 层与 preload
  - [ ] 4.1 rental.handler 注册 rental.* 通道与校验
  - [ ] 4.2 preload 暴露 window.api.rental
  - [ ] 4.3 编写校验函数与 IPC 单测

- [ ] 5. 出租管理页面
  - [ ] 5.1 出租房管理 tab（登记/编辑/删除，关联固定资产）
  - [ ] 5.2 租户管理 tab（登记/编辑/删除，关联合同拦截提示）
  - [ ] 5.3 合同管理 tab（创建/编辑/终止，收租记录与累计租金展示）
  - [ ] 5.4 收租 tab（记录收租选择合同与收入账户，删除收租）
  - [ ] 5.5 新增 /rentals 路由与侧边栏入口
  - [ ] 5.6 编写 RentalsView 组件测试

- [ ] 6. 仪表盘提醒
  - [ ] 6.1 合同剩余 30 天到期提醒
  - [ ] 6.2 应收租日（付租周期到期前 3 天）提醒
  - [ ] 6.3 编写提醒逻辑纯函数单元测试
